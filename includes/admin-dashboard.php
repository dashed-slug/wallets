...
if ( ! class_exists( 'Dashed_Slug_Wallets_Admin_Dashboard' ) ) {
	class Dashed_Slug_Wallets_Admin_Dashboard {
		...
		public function enqueue_scripts() {
			if ( file_exists( DSWALLETS_PATH . '/assets/scripts/wallets-admin-dashboard-5.0.15.min.js' ) ) {
				$script = 'wallets-admin-dashboard-5.0.15.min.js';
			} else {
		...
		public function tab_debug_cb() {
			...
			$data = array();
			$data[ __( 'Plugin version', 'wallets' ) ]         = '5.0.15';
			$data[ __( 'Git SHA', 'wallets' ) ]                = 'cd96aba5';
