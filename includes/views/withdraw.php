...
<form class="dashed-slug-wallets withdraw" data-bind="submit: doWithdraw, if: coins().length">
	...
	<label class="amount"><?php esc_html_e( 'Amount', 'wallets-front' ); ?>: <input type="text"  data-bind="value: withdrawAmount, valueUpdate: ['afterkeydown', 'input']" /></label>
	<label class="fee"><?php esc_html_e( 'Fee (deducted from amount)', 'wallets-front' ); ?>: <input type="text" data-bind="value: withdrawFee, enable: false" /></label>
	<label class="comment"><?php esc_html_e( 'Transaction comment', 'wallets-front' ); ?>: <input type="text"  data-bind="value: withdrawComment" /></label>
