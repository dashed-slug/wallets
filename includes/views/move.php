...
<form class="dashed-slug-wallets move" data-bind="submit: doMove, if: coins().length">
	...
	<label class="amount"><?php esc_html_e( 'Amount', 'wallets-front' ); ?>: <input type="text"  data-bind="value: moveAmount, valueUpdate: ['afterkeydown', 'input']" /><span class="base-amount" data-bind="text: moveBaseAmount" ></span></label>
	<label class="fee"><?php esc_html_e( 'Fee (deducted from amount)', 'wallets-front' ); ?>: <input type="text" data-bind="value: moveFee()[0], enable: false" /><span class="base-amount" data-bind="text: moveFee()[1]" ></span></label>
	<label class="comment"><?php esc_html_e( 'Transfer comment', 'wallets-front' ); ?>: <textarea data-bind="value: moveComment"></textarea></label>
	<hr />
