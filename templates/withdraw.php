...
<form class="dashed-slug-wallets withdraw" data-bind="submit: doWithdraw, css: { 'wallets-ready': !coinsDirty() && ajaxSemaphore() < 1, 'fiat-coin': selectedCryptoCoin() && coins()[ selectedCryptoCoin() ].is_fiat, 'crypto-coin': selectedCryptoCoin() && coins()[ selectedCryptoCoin() ].is_crypto }">
	<?php
		...
		<tbody>
			...
			<tr>
				<td colspan="6" style="position: relative;">
					<?php if ( 'on' === Dashed_Slug_wallets::get_option( 'wallets_qrcode_enabled' ) ): ?>
					<label
					...
					<label class="address"><?php echo apply_filters( 'wallets_ui_text_withdrawtoaddress', esc_html__( 'Withdraw to address', 'wallets-front' ) ); ?>: <input type="text" class="qrcode-text" required="required" data-bind="value: withdrawAddress" /></label>
					<?php else: ?>
