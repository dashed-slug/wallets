<?php defined( 'ABSPATH' ) || die( -1 ); // don't load directly ?>
	...
	<form class="dashed-slug-wallets transactions transactions-rows" data-bind="css: { 'wallets-ready': ! transactionsDirty() }" onsubmit="return false;">
		...
		<ul data-bind="visible: transactions().length, foreach: transactions()">
			<li data-bind="if: category == 'withdraw', visible: category == 'withdraw', css: { unconfirmed: status == 'unconfirmed', pending: status == 'pending', done: status == 'done', failed: status == 'failed', cancelled: status == 'cancelled', 'fiat-coin': $root.coins()[ symbol ] && $root.coins()[ symbol ].is_fiat, 'crypto-coin': $root.coins()[ symbol ] && $root.coins()[ symbol ].is_crypto }" class="withdraw">
			...
			<li data-bind="if: category == 'deposit', visible: category == 'deposit', css: { unconfirmed: status == 'unconfirmed', pending: status == 'pending', done: status == 'done', failed: status == 'failed', 'fiat-coin': $root.coins()[ symbol ] && $root.coins()[ symbol ].is_fiat, 'crypto-coin': $root.coins()[ symbol ] && $root.coins()[ symbol ].is_crypto }" class="deposit">
				<div class="type label"><?php echo apply_filters( 'wallets_ui_text_type', esc_html__( 'Type', 'wallets-front' ) ); ?>: <span data-bind="text: wallets_ko_i18n[ category ]"></span></div>
				<div class="time"><time data-bind="text: moment( created_time + '.000Z' ).toDate().toLocaleString(), attr: { datetime: created_time + 'Z' }"></time></div>
			...
			<li data-bind="if: category == 'move', visible: category == 'move', css: { unconfirmed: status == 'unconfirmed', pending: status == 'pending', done: status == 'done', failed: status == 'failed', 'fiat-coin': $root.coins()[ symbol ] && $root.coins()[ symbol ].is_fiat, 'crypto-coin': $root.coins()[ symbol ] && $root.coins()[ symbol ].is_crypto }" class="move">
				<div class="type label"><?php echo apply_filters( 'wallets_ui_text_type', esc_html__( 'Type', 'wallets-front' ) ); ?>: <span data-bind="text: wallets_ko_i18n[ category ]"></span></div>
				<div class="time"><time data-bind="text: moment( created_time + '.000Z' ).toDate().toLocaleString(), attr: { datetime: created_time + 'Z' }"></time></div>
			...
			<li data-bind="if: category == 'trade', visible: category == 'trade', css: { unconfirmed: status == 'unconfirmed', pending: status == 'pending', done: status == 'done', failed: status == 'failed', 'fiat-coin': $root.coins()[ symbol ] && $root.coins()[ symbol ].is_fiat, 'crypto-coin': $root.coins()[ symbol ] && $root.coins()[ symbol ].is_crypto }" class="trade">
				<div class="type label"><?php echo apply_filters( 'wallets_ui_text_type', esc_html__( 'Type', 'wallets-front' ) ); ?>: <span data-bind="text: wallets_ko_i18n[ category ]"></span></div>
				<div class="time"><time data-bind="text: moment( created_time + '.000Z' ).toDate().toLocaleString(), attr: { datetime: created_time + 'Z' }"></time></div>
