...
/**
 * Email sent to the recipient of a deposit that is now cancelled (cancelled state/draft post status and empty wallets_error post meta).
 *
...
<?php
	esc_html_e(
		sprintf(
			...
			__(
				'You were about to receive a %1$s deposit of %2$s from address %3$s.',
				'wallets'
			),
			$tx->currency->name,
...
<p><?php esc_html_e( 'The transaction has now been cancelled by an admin.', 'wallets' ); ?></p>
