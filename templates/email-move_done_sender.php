...
/**
 * Email sent to the sender of an internal transaction (move) that has been executed (done state/published post status).
 *
	...
	<?php
		esc_html_e(
			sprintf(
				...
				__(
					'You have sent a %1$s amount of %2$s to user %3$s.',
					'wallets'
				),
				$tx->currency->name,
	...
	<?php
		esc_html_e(
			sprintf(
				...
				__(
					'The %1$s amount of %2$s has been deducted from your account.',
					'wallets'
				),
				$tx->currency->name,
				$tx->get_amount_as_string( 'amount', false, true )
